...
**Answer:** The ICC for schools is $\frac{\sigma_{\zeta_0}^2}{\sigma_{\zeta_0}^2+\sigma_\varepsilon^2}=$`r as.data.frame(VarCorr(fit1))$vcov[2]/(as.data.frame(VarCorr(fit1))$vcov[2]+as.data.frame(VarCorr(fit1))$vcov[3])` and the ICC for classrooms is $\frac{\sigma_{\eta_0}^2}{\sigma_{\eta_0}^2+\sigma_\varepsilon^2}=$`r as.data.frame(VarCorr(fit1))$vcov[1]/(as.data.frame(VarCorr(fit1))$vcov[1]+as.data.frame(VarCorr(fit1))$vcov[3])`.  
...
summary(fit5.3)
```
**Answer 5b:** The model with random slope on `mathknow` and the model with random slope on `mathprep` have convergent problem.
...
fit5.c.2 <- lmer( math1st ~ ses + minority + sex + yearstea + mathknow + mathprep + 
                  ...
                  dat, subset = in_sample)
fit5.c.2 <- lmer( math1st ~ ses + minority + sex + yearstea + mathknow + mathprep + 
                    housepov + (mathknow| classid) + (1 | schoolid/classid), 
                  dat, subset = in_sample)
summary(fit5.c.2)
```
...
VarCorr(fit5.c.3)
```
It seems that in all these models, the random slope and random intercepts are highly correlated. After introducing correlation between random slope and random intercept, we found that school-level random intercept increase a lot in the model with `mathprep` or `yearstea`, while it decrease in the model with `mathknow`.
...
anova(fit8.a.1, fit9) #P = 0.1581
anova(fit8.a.2, fit9) #P = 0.02248
```
